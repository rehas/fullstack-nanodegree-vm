...
CLIENT_ID = json.loads(
    open('localsafe/client_secrets.json', 'r').read())['web']['client_id']

FB_APP_ID = json.loads(
    open('localsafe/fb_client_secrets.json', 'r').read())['web']['app_id']
FB_APP_SECRET = json.loads(
    open('localsafe/fb_client_secrets.json', 'r').read())['web']['app_secret']
...
def gconnect():
    ...
    try:
        ...
        oauth_flow = flow_from_clientsecrets('localsafe/client_secrets.json', scope='')
...
def gdisconnect():
    ...
    print("TOKEN =>  %s" % login_session['access_token'])
    ...
    if result['status'] == '200':
        ...
        response = make_response(json.dumps('Failed to revoke token for this user'), 200)
        response.headers['Content-Type'] = 'application/json'
        return response         
...
def fbconnect():
    ...
    userinfo_url = url = 'https://graph.facebook.com/v2.8/me?'
    token = result.split("&")[0]

    url = url = 'https://graph.facebook.com/v2.8/me?\
...
def editRestaurant(restaurant_id):
    ...
    if editedRestaurant.user_id != login_session['user_id']:
        return "\
            ...
            To delete a restaurant you must craete it');}</script> \
...
##TODO: Implement user authorization check for all edit/delete operations
...
def editMenuItem(restaurant_id, menu_id):
    ...
    if editedItem.user_id != login_session['user_id']:
        return "\
            ...
            To edit a menu item you must craete it');}</script> \
            <body onload='myfunction()'></body> \
        "
